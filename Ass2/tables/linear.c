...
struct linear_table {
	...
	int collisions; // for stats collisions
	int probe;      // for stats, probing
};
...
bool linear_hash_table_insert(LinearHashTable *table, int64 key) {
	...
	table->probe = table->probe + steps;
	//calculating the number of collisions
	table->collisions=table->collisions+collided;
...
void linear_hash_table_stats(LinearHashTable *table) {
	...
	printf("Length of average probe sequence  %f\n", ((table->probe)*1.0)/((table->load)*1.0));
	printf("--- end stats ---\n");
}
